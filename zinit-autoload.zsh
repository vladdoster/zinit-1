...
.zinit-self-update() {
    ...
    [[ $1 = -q ]] && +zinit-message -n "{pre}[self-update]{rst}:{info} Updating {cmd}Zinit{info} repository{rst}" \
    ...
    if [[ $1 != -q ]] {
        +zinit-message "{pre}[self-update]{rst}:{info} Compiling Zinit via {cmd}zcompile{rst}"
    }
